...
<div class="container py-4 text-dark">
    ...
    <div class="d-flex justify-content-between align-items-center mb-4">
        <div>
            <a href="${pageContext.request.contextPath}/admin/orders" class="text-decoration-none small text-muted">
    ...
    <div class="row g-4">
        ...
        <div class="col-lg-8">
            ...
            <div class="card border-0 shadow-sm mb-4">
                <div class="card-header bg-white py-3">
                    <h5 class="mb-0 fw-bold"><i class="fas fa-user-circle me-2 text-primary"></i>Thông tin người nhận</h5>
                </div>
                <div class="card-body">
                    <div class="row g-3">
                        <div class="col-md-6">
                        ...
                        <c:if test="${not empty order.note}">
                            <div class="col-12">
                                <label class="text-muted small d-block">Ghi chú từ khách</label>
                                <div class="p-2 bg-light rounded italic border-start border-primary border-3">"${order.note}"</div>
                            </div>
                        </c:if>
                    </div>
                </div>
            ...
            <div class="card border-0 shadow-sm mb-4">
                <div class="card-header bg-white py-3">
                    <h5 class="mb-0 fw-bold"><i class="fas fa-shopping-basket me-2 text-success"></i>Sản phẩm đơn hàng</h5>
                </div>
                ...
                <div class="card-footer bg-white py-3 text-dark">
                    ...
                    <c:if test="${not empty order.couponCode}">
                        <div class="d-flex justify-content-between mb-1 text-success">
                            ...
                            <span>-${order.discountAmountDisplay}</span>
                        </div>
                    </c:if>
        ...
        <div class="col-lg-4">
            ...
            <div class="card border-0 shadow-sm mb-4">
                <div class="card-header bg-white py-3">
                    <h6 class="mb-0 fw-bold"><i class="fas fa-truck me-2 text-info"></i>Vận chuyển</h6>
                </div>
                <div class="card-body">
                    <div class="mb-3">
                        <label class="small text-muted d-block">Phương thức</label>
                        <span class="fw-bold text-uppercase">${order.shippingMethod}</span>
                    </div>
                    <div class="mb-3">
                        <label class="small text-muted d-block">Trạng thái vận đơn</label>
                        <span class="badge bg-info text-white p-2 mt-1">
                             <i class="fas fa-box me-1"></i> ${order.shippingStatusDisplay}
                        </span>
                    </div>
                    
                    <%-- Thông tin Shipper Snapshot --%>
                    <div class="p-3 bg-light rounded border">
                        <label class="small text-muted fw-bold d-block mb-2 text-dark">Shipper phụ trách</label>
                        <c:choose>
                            <c:when test="${not empty order.shipperName and order.shipperName != 'N/A'}">
                                <div class="d-flex align-items-center">
                                    <div class="bg-primary text-white rounded-circle p-2 me-2">
                                        <i class="fas fa-user-check"></i>
                                    </div>
                                    <span class="fw-bold text-dark">${order.shipperName}</span>
                                </div>
                            </c:when>
                            <c:otherwise>
                                <span class="text-warning italic small">
                                    <i class="fas fa-hourglass-half me-1"></i> Đang chờ điều phối...
                                </span>
                            </c:otherwise>
                        </c:choose>
                    </div>
                    ...
                    <c:if test="${order.status == 'CONFIRMED' and (empty order.shipperName or order.shipperName == 'N/A')}">
                        <form action="${pageContext.request.contextPath}/admin/shipping/${order.id}/re-dispatch" method="post" class="mt-3">
                            <input type="hidden" name="${_csrf.parameterName}" value="${_csrf.token}"/>
                            <button type="submit" class="btn btn-outline-warning btn-sm w-100 fw-bold">
                                <i class="fas fa-redo me-1"></i> Thử tìm lại Shipper
                            </button>
                        </form>
                    </c:if>
                </div>
                ...
                <div class="card-body">
                    ...
                    <c:choose>
                        ...
                        <c:when test="${order.status == 'PENDING'}">
                            <form method="post" action="${pageContext.request.contextPath}/admin/orders/${order.id}/status">
                                ...
                                <p class="small text-muted mb-3 italic">
                                    Xác nhận đơn hàng để hệ thống tự động trừ kho và gán Shipper giao hàng.
                                </p>
                                
                                <button class="btn btn-primary w-100 fw-bold py-2 mb-2" type="submit">
                        ...
                        <c:when test="${order.status == 'CONFIRMED'}">
                            <div class="alert alert-success border-0 bg-success bg-opacity-10 text-success small mb-0">
                                <i class="fas fa-info-circle me-1"></i> Đơn hàng đang trong quá trình vận chuyển. Không thể thay đổi.
                            </div>
                        </c:when>

                        <c:otherwise>
                            <div class="alert alert-secondary border-0 small mb-0 text-center">
                    ...
                    <div class="d-flex justify-content-between align-items-center">
                        ...
                        <span class="badge ${order.paymentStatusDisplay == 'PAID' ? 'bg-success' : 'bg-warning text-dark'}">
