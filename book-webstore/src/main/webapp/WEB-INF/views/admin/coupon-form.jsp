...
<div class="container py-4">
    ...
    <div class="card border-0 shadow-sm">
        ...
        <div class="card-body p-4">
            <form method="post" action="${pageContext.request.contextPath}/admin/coupons/save" class="row g-4">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" class="form-control fw-bold text-primary" name="code" 
                           ...
                           ${coupon.id != null ? 'readonly' : ''}>
                    <div class="form-text">Mã định danh duy nhất để khách hàng nhập vào.</div>
                </div>
                ...
                <div class="col-md-4">
                    ...
                    <div class="input-group">
                        <input type="number" id="discountValue" step="1" min="1" class="form-control" 
                ...
                <div class="col-md-3">
                    ...
                    <input type="number" step="1000" min="0" class="form-control" 
                           name="minOrderValue" value="${coupon.minOrderValue}">
                </div>
                ...
                <div class="col-md-4 d-flex align-items-end px-4">
                    <div class="form-check form-switch p-3 bg-light rounded w-100 border">
                        <input class="form-check-input ms-0 me-2" type="checkbox" id="active" name="active" 
                               value="true" ${coupon.active ? 'checked' : ''}>
                        <label class="form-check-label fw-bold text-success" for="active">Kích hoạt Coupon ngay</label>
                    </div>
                </div>

                <div class="col-12">
                    <label class="form-label fw-bold small text-muted">Sách được áp dụng (Chọn nhiều)</label>
                    <select class="form-select" name="applicableBookIds" multiple size="8">
                        <c:forEach var="book" items="${books}">
                            ...
                            <c:forEach var="selectedId" items="${coupon.applicableBookIds}">
                                <c:if test="${selectedId == book.id}"><c:set var="isSelected" value="true" /></c:if>
                            </c:forEach>
                            <option value="${book.id}" ${isSelected ? 'selected' : ''}>${book.title} (ID: ${book.id})</option>
                        </c:forEach>
                    </select>
                    <div class="form-text text-info"><i class="fas fa-info-circle me-1"></i> Giữ phím Ctrl (hoặc Cmd) để chọn nhiều sách. Để trống nếu áp dụng cho toàn bộ cửa hàng.</div>
                </div>
                ...
                <div class="col-12 d-flex gap-2 pt-3 border-top mt-4">
                    ...
                    <a href="${pageContext.request.contextPath}/admin/coupons" class="btn btn-light border px-4 py-2">Hủy bỏ</a>
                </div>
            </form>
        </div>
    </div>
